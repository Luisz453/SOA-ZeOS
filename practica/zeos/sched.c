...
struct shm_mem_struct shm_mem[10];
...
void init_shared_memory()
{
  ...
  for(int i = 0; i < length; ++i){
    shm_mem[i].shm_mem_frame_addr = 0;
    int new_ph_pag = alloc_frame();
    if (new_ph_pag == -1) // Hay un error y solo avisaremos al usuario
    {
      printk("Error al alocatar las paginas");
    } else {
      shm_mem[i].shm_mem_frame_addr = new_ph_pag;
      shm_mem[i].referencias = 0;
    }
  }
}
