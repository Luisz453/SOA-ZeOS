...
int access_ok(int type, const void * addr, unsigned long size)
{
  ...
  int pag_to_loop = (addr_fin - addr_ini);
  if (size % PAGE_SIZE != 0) pag_to_loop++;
  ...
  switch(type)
  {
    case VERIFY_WRITE:
      ...
      if ((addr_ini>USER_FIRST_PAGE+NUM_PAG_CODE+NUM_PAG_DATA) && (addr_fin<TOTAL_PAGES)){
        for(int i = 0; i < pag_to_loop; ++i){
          int frame = get_frame(page_table, addr_ini+i);
          if(frame == 0) return 0;
        }
        return 1;
      }
    default:
      if ((addr_ini>=USER_FIRST_PAGE) && (addr_fin<TOTAL_PAGES))
        for(int i = 0; i < pag_to_loop; ++i){
          int frame = get_frame(page_table, addr_ini+i);
          if(frame == 0) return 0;
        }
